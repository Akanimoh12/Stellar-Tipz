...
const {
  ...
  mockTriggerManualJob: vi.fn(),
}))

vi.mock('./manual-jobs.service.js', () => ({
  MANUAL_JOB_NAMES: [
    'credit-recompute',
    'analytics-daily',
    'subscription-charge',
...
describe('POST /api/v1/admin/audit-log', () => {
  it('returns 401 without authorization', async () => {
    const res = await request(createApp()).post('/api/v1/admin/audit-log').send({ action: 'test' })
...
describe('POST /api/v1/admin/jobs/:name/trigger', () => {
  it('returns 401 without authorization', async () => {
    const res = await request(createApp())
      .post('/api/v1/admin/jobs/analytics-daily/trigger')
      .send({ params: { date: '2026-09-24' } })
  ...
  it('allows an admin to trigger a scheduled job with parameters', async () => {
    mockAuth('admin-1', 'admin')
    mockTriggerManualJob.mockResolvedValueOnce({
    ...
    expect(mockTriggerManualJob).toHaveBeenCalledWith('analytics-daily', 'admin-1', {
      date: '2026-09-24',
    })
  })
  ...
  it('rejects an unknown job name', async () => {
    mockAuth('admin-1', 'admin')
    const res = await request(createApp())
      .post('/api/v1/admin/jobs/not-a-job/trigger')
      .set('Authorization', 'Bearer valid-token')
      .send({})
