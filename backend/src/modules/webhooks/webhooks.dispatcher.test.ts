...
describe('dispatchWebhookEvent (issue #998)', () => {
  beforeEach(() => {
    ...
    vi.mocked(prisma.webhookDelivery.create)
      .mockResolvedValueOnce({ id: 'delivery-wh_sub_a' } as never)
      .mockResolvedValueOnce({ id: 'delivery-wh_sub_b' } as never)
  })
  ...
  it('sends a signed envelope with an ISO timestamp so consumers can verify authenticity', async () => {
    ...
    expect(() =>
      new Date((envelope as { timestamp: string }).timestamp).toISOString(),
    ).not.toThrow()
  })
